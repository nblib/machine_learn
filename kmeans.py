...
def testBasicFunc():
    # 加载测试数据集
    dataMat = mat(loadDataSet(os.getcwd() + "/data/kmeans_data.txt"))
...
def testKMeans():
    ...
    plotBestFit(dataMat,myCentroids)
...
def testBiKMeans():
    ...
    centList, myNewAssments = biKMeans(dataMat, 5)

    print 'centList=', centList
    plotBestFit(dataMat,centList)

# 可视化展示
def plotBestFit(dataArr, labelMat):
    ...
    m = shape(labelMat)[0]
    for i in range(m):
        xcord2.append(labelMat[i, 0])
        ycord2.append(labelMat[i, 1])
    fig = plt.figure()
